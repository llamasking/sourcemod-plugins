...
// Blacklist certain people from abusing this system.
// Code borrowed from motdgd since that has a black/whitelist feature and I barely know SP.
public void LoadBlacklist ()
{
    if (g_blacklist)
    {
        ClearArray(g_blacklist);
    }
    else
    {
        g_blacklist = CreateArray(32);
    }

    char blackPath[PLATFORM_MAX_PATH];
    BuildPath(Path_SM, blackPath, sizeof(blackPath), "configs/customvotes_blacklist.cfg");
    Handle blackFile = OpenFile(blackPath, "r");
    if (!blackFile)
    {
        //LogMessage("Blacklist file not located.")
        return;
    }

    char SteamID[32];
    while(ReadFileLine(blackFile, SteamID[8], sizeof(SteamID)))
    {
        ReplaceString(SteamID[8], sizeof(SteamID), "\n", "", true);
        PushArrayString(g_blacklist, SteamID[8]);
        //LogMessage("Added %s to blacklist", SteamID[8])
    }

    CloseHandle(blackFile);
}
...
public Menu_PlayersVote(iVote, iVoter)
{
    ...
    char SteamID[32];
    GetClientAuthId(iVoter, AuthId_Steam2, SteamID, sizeof(SteamID));

    if (FindStringInArray(g_blacklist, SteamID[8]) != -1)
    {
        CPrintToChat(iVoter, "[SM] %t", "No Access");
        return;
    }
